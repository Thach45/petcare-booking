...
export function SiteHeader() { const pathname = usePathname(); const [open, setOpen] = useState(false); return <header className="site-header"><div className="nav-shell"><PetCareLogo /><nav className={open ? "nav-links open" : "nav-links"} aria-label="Điều hướng chính">{links.map((link) => <Link key={link.href} href={link.href} className={pathname === link.href ? "active" : ""} onClick={() => setOpen(false)}>{link.label}</Link>)}</nav><div className="nav-actions"><NotificationBell /><Link className="admin-link header-link" href="/admin">Quản trị</Link><Link className="login-button header-link" href="/login">Đăng nhập</Link><Link className="join-button header-link" href="/register">Đăng ký</Link><button className="icon-button menu-button" onClick={() => setOpen((value) => !value)} aria-expanded={open} aria-label={open ? "Đóng menu" : "Mở menu"}>{open ? "×" : <svg aria-hidden="true" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M4 7h16M4 12h16M4 17h16" /></svg>}</button></div></div></header>; }
